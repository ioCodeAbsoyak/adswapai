"""AdSwapAI R&D, 2025-08-25: SAM2 video predictor baseline, grid-point prompts propagated through a clip."""
...
import sys
SAM2_REPO = Path(__file__).resolve().parent / "sam2"  # https://github.com/facebookresearch/sam2
...
def create_output_video(frame_paths, video_segments, fps, frame_size, output_path):
    ...
    for frame_idx, frame_path in enumerate(frame_paths):
        ...
        if frame is None:
            print(f"Could not read frame {frame_idx}: {frame_path}")
            continue
                ...
                if obj_id < len(colors):
                    ...
                    if isinstance(mask, np.ndarray):
                        ...
                        if mask.shape[:2] == frame.shape[:2]:  # H, W should match
                            ...
                            print(f"Mask shape {mask.shape} doesn't match frame shape {frame.shape}")
                    else:
                        print(f"Invalid mask type for object {obj_id}")
...
def main():
    ...
    if len(detected_object_ids) == 0:
        print("No objects detected, but proceeding with video creation...")
