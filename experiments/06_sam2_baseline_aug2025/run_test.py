"""AdSwapAI R&D, 2025-08-25: SAM2 image predictor baseline, auto / point / box prompt from config.yaml."""
...
def main():
    ...
    # Select predictor based on prompt type
    prompt = cfg["prompt"]
    if prompt["type"] == "auto":
    ...
    # GPU memory status (if CUDA is being used)
    ...
    if len(masks) > 1:
        ...
        if isinstance(mask, np.ndarray):
            ...
            m = (mask > 0.5).astype("uint8") * 255
        overlay = img.copy()
        overlay[m == 255] = (0, 255, 0)  # green mask
